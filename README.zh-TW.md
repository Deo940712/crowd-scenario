...
你已經做了一個決定（要推一次 breaking 大改版、要漲價、要升息）。你想**排練**一下群眾
可能怎麼反應——不是預測未來，而是拿一個「合成的第二意見」來壓力測試你自己的判讀。

`crowd-scenario` 接收一個「分桶後的情境快照」，然後讓一群**人格原型（persona）**上場——
每個原型有自己的語氣、自己的敏感度——對這個事件做出反應。引擎把他們排成一條
**「誰先動」的反應鏈**（嘗鮮的先動，保守的慢慢等），最後只吐出**一個類別性立場**
（`負面 | 中性 | 正面`）加上一段可讀的敘事。
...
```text
        你的情境                    引擎                        產出
 ┌────────────────────┐  分桶    ┌──────────────────────┐    ┌───────────────────┐
 │ 破壞性     0.95    │────────▶│ 8 個人格各自反應,    │───▶│ 立場：抗拒        │
 │ 遷移成本   0.9     │(數字在此 │ 排成「誰先動」的     │    │ + 反應鏈敘事      │
 │ 事件：v9 大改版    │ 被丟棄)  │ 反應鏈,全程決定性    │    │                   │
 └────────────────────┘          └──────────────────────┘    └───────────────────┘
       原始數字到此為止 ↑                      沒有任何決策等級純量離開 ↑
```

> **精確的宣稱。**「沒有數字離開」是簡稱。契約真正保證的是：**沒有決策等級的可加總
> 純量**（score / weight / modifier / 預期報酬）、**沒有原始指標數字**（價格 / 殖利率 /
> 淨值 / 遷移成本 / 流失率）出現在產出物上。有限的類別性整數*確實*會出現——人格
> `stance ∈ {-1,0,1}`、`narrative_intensity ∈ {1,2,3}`、序號列表——但它們是固定有限集合的
> 標籤，不是任何東西能加總進決策的數值。
